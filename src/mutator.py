...
class Mutator(object):
  ...
  def checkCSingle(self, module, claz, fname, ofunc, mfunc, i):
    mv = None
    ov = None
    try:
  ...
  def checkCEquivalence(self, module, claz, fname, ofunc, mfunc, checks):
    nvars = ofunc.func_code.co_argcount
    myargnames = ofunc.func_code.co_varnames[0:nvars]
    struct = self.evalChecks(myargnames,checks)
    space = samplespace.SampleSpace(config.config['MaxSpace'], config.config['MaxTries'])
    myargs = space.genArgs(struct)
    for arginst in myargs:
      res = self.checkCSingle(module, claz, fname, ofunc, mfunc, arginst)
      if not(res): return False
    return True
  ...
  def runCTests(self, module, claz, function, not_covered, skip_ops, checks):
    mutant_count = 0
    detected = 0
    equivalent = 0
    not_equivalent = 0
    ...
    res = mpool.parmap(self.evalCMutant, tomap)
    eqv = []
    for (ret,m) in zip(res, tomap):
      if ret == config.FnRes['TimedOut']:
        pass
